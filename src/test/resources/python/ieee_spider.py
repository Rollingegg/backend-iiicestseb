...
class IeeeSpider:
    def __init__(self, search_conference: str, start_year: str, end_year: str, output_file: str):
        ...
        self.out_file = open(output_file, "w+", encoding='utf-8')
    ...
    def get_reference(self, url: str):
        ...
        if r.status_code == requests.codes.ok:
            content = r.json()
            for ref in content.get('references', list()):
                reference.append({
                    "order": ref.get('order', ''),
                    "title": ref.get('title', ''),
                    "text": ref.get('text', ''),
                    "googleScholarLink": ref.get('googleScholarLink', ''),
